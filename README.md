...
- see the 'Bayesian Ideal Observer' section of the Methods in (https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5700427/) for a mathematical description of what the 'IO.m' function does for a different task and stimulus set. 
...
- Similar to the IO, the task that is being simulated can be changed by changing the stimulus set and and the parameter for the specified number of classes as needed. 
...
- The script saves the results of the performance map in a directory called 'SaveDir_FIO_batches_gender', along with intermediate variables such as the likelihoods found after a set of trials, as well as the initial parameters used to run the function.
...
##### FIO_smallSet.m, runFIO_smallSet.m, and fit_params_FIO

- The 'runFIO_smallSet.m' script runs the 'FIO_smallSet.m' function for a face identification task with 10 male faces that are saved in the 'human_pics.mat' file. 
- The script saves the results of the performance map in a directory called 'SaveDir_IdTask', along with intermediate variables such as the likelihoods found after a set of trials, as well as the initial parameters used to run the function.
- The script contains parameter settings that can be changed for the spatially variant contrast sensitivity function that is used to filter the images in order to simulate a foveated visual system.  
- The 'fit_params_FIO.m' script can be used to find new parameters to fit human psychophysics performance data from different fixation positions. The closeness of fit is measured using the Akaike Information Criterion (AIC). New performance data and fixation positions to fit the model to can be changed on lines 5-7. The parameter search is done in a fairly narrow range that has empirically worked for different face discrimination tasks. However, these parameter ranges can be changed in the script if the minimum AIC is found at the edge of the current range of parameter settings (which indicates that a better set of parameters is outside of this range). 
- The 'FIO_smallSet.m' function is meant to be used with small stimulus sets (smaller than about 15 500x500px images). The actual size of the stimulus set that this function can process without crashing depends on the size of the RAM on the computer that you're using. This function does not save filtered templates after calculating them, but instead keeps them in RAM the whole time, until they are needed to calculate the mean and covariance matrices. 
- Similar to the IO, the task that is being simulated can be changed by changing the stimulus set and and the parameter for the specified number of classes as needed.
- see the 'Supplementary Materials' of (https://www.pnas.org/content/pnas/suppl/2012/11/07/1214269109.DCSupplemental/pnas.201214269SI.pdf) for a mathematical description of the spatially variant contrast sensitivity function along with a mathematical description of the FIO for this stimulus set. Also see the see 'S-FIO model' section of the Methods in (https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5700427/) for a mathematical description of a different version of the FIO with a different stimulus set. 
